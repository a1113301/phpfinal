...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 檢查資料庫中是否已經存在相同的類別
    $check_query = "SELECT * FROM category WHERE catname = '$category'";
    ...
    if ($check_result->num_rows > 0) {
        ...
        // 插入新類別到資料庫
        $insert_query = "INSERT INTO category (catname) VALUES ('$category')";
        if ($conn->query($insert_query) === TRUE) {
            $message = "類別新增成功";
        } else {
            $message = "類別新增失敗: " . $conn->error;
        }
    }
}
...
<div class="main-content">
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
        <label for="category">類別名稱:</label>
        <input type="text" id="category" name="category" required>
        <input type="submit"  class="button primary"value="新增類別">
    </form>
</div>
</body>
</html>
